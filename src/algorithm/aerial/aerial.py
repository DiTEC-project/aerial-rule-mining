...
class Aerial:
        ...
        for r in range(1, self.max_antecedents + 1):
            ...
            if batch_vectors:
                ...
                for test_vector, implication_probabilities, candidate_antecedents \
                    ...
                    # dynamic support threshold: (1 / (category["end"] - category["start"]))
    ...
    def generate_frequent_itemsets(self):
        """
        Generate frequent itemsets using the Aerial algorithm (optimized version).
        """
        frequent_itemsets = []
        input_vector_size = len(self.input_vectors['vector_tracker_list'])
    ...
    def calculate_stats(self, rules, transactions, exec_time):
        ...
        for rule_index in range(len(rules)):
            ...
            for index in range(len(self.input_vectors['vector_list'])):
                ...
                if encoded_transaction[self.input_vectors['vector_tracker_list'].
        ...
        stats["coverage"] = sum(dataset_coverage) / len(self.input_vectors['vector_list'])
        return [len(rules), exec_time, stats['support'], stats["confidence"], stats["coverage"],
                stats["zhangs_metric"]], rules
    ...
    def train_ae_model(self, loss_function=torch.nn.BCELoss(), lr=5e-3, epochs=1, batch_size=128):
        """
