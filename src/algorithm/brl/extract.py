...
def extract_itemsets(X, min_support=0.1, max_cardinality=2, verbose=False) -> (List[Tuple], float):
    itemsets_df = mlx.fpgrowth(X, min_support=min_support, max_len=max_cardinality)
    average_support = itemsets_df["support"].mean()
    ...
    return itemsets, average_support
